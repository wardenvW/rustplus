...
async def run_bot(socket: RustSocket, tracking: TrackedList, server_details: RustServer, logger: logging.Logger):
    ...
    try:
        ...
        @Command(server_details)
        async def crate(command: ChatCommand):
            if event_handler.crates is None:
                await socket.send_team_message(f"{Emoji.EXCLAMATION}Crate hasn't been active yet.")
                return
            
            await event_handler.crates.get_info()
