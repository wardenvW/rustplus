...
from ..rust_models import RustTime
from .emojis import Emoji

import requests
from PIL import ImageFont, Image, ImageDraw

ICONS_PATH = "rustWplus.resources.icons"
FONT_PATH = "rustWplus.resources.fonts"
...
def format_time_simple(rust_time: RustTime) -> str:
    current = convert_time(rust_time.raw_time)
    current_min = rust_time.raw_time * 60

    sunrise_min = parse_time(rust_time.sunrise)
    sunset_min = parse_time(rust_time.sunset)
    
    if sunrise_min <= current_min < sunset_min:
        game_min = sunset_min - current_min
        event = "Sunset"
    else:
        game_min = sunrise_min - current_min if current_min < sunrise_min else (1440 - current_min) + sunrise_min
        event = "Sunrise"
    
    real_min = int(game_min * (rust_time.day_length / 1440))
    
    return f"{Emoji.EXCLAMATION}Time {current} ({real_min}m to {event})"
...
def generate_grid(map_size: int, text_size: int = 20, text_padding: int = 5, color: str = "black") -> Image.Image:
    img = Image.new("RGBA", (map_size, map_size), (0, 255, 0, 0))
    ...
    for col in range(num_cells):
        for row in range(num_cells):
            start = (col * GRID_DIAMETER, row * GRID_DIAMETER)
            end = ( (col + 1) * GRID_DIAMETER, (row + 1) * GRID_DIAMETER)
            d.rectangle(xy=(start, end), outline=color) 

            text = letters[col] + str(row)
...
def format_coords(x: int, y: int, map_size: int) -> Tuple[int, int]:
...
async def fetch_avatar_icon(steam_id: int, online: bool) -> Image.Image:
    avatar = (
        Image.open(
        requests.get(f"https://companion-rust.facepunch.com/api/avatar/{steam_id}", stream=True).raw
    )
...
async def avatar_processing(image: Image.Image, border_size: int, player_online: bool = False) -> Image.Image:
    size_with_border = (
